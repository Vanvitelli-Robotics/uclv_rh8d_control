...
using namespace hand_joint_trajectory_controller::interpolation_methods;  // NOLINT
...
TEST(TestTrajectory, initialize_trajectory)
{
  auto clock = rclcpp::Clock(RCL_STEADY_TIME);
  {
    ...
    auto traj = hand_joint_trajectory_controller::Trajectory(empty_msg);
    EXPECT_EQ(0, traj.last_sample_index());

    trajectory_msgs::msg::JointTrajectoryPoint expected_point;
    hand_joint_trajectory_controller::TrajectoryPointConstIter start, end;
...
TEST(TestTrajectory, sample_trajectory_positions)
{
  ...
  auto traj = hand_joint_trajectory_controller::Trajectory(time_now, point_before_msg, full_msg);
...
TEST(TestTrajectory, interpolation_pos_vel)
{
  ...
  auto traj = hand_joint_trajectory_controller::Trajectory();
...
TEST(TestTrajectory, update_trajectory)
{
  ...
  const rclcpp::Time time_now = rclcpp::Clock().now();
  auto traj = hand_joint_trajectory_controller::Trajectory(time_now, point_before_msg, first_msg);
  EXPECT_EQ(0, traj.last_sample_index());
...
TEST(TestWrapAroundJoint, no_wraparound)
{
  ...
  hand_joint_trajectory_controller::wraparound_joint(
